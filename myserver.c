...
void sendMsgtoClient(int clientFD, char *str) {
    ...
    int n = write(clientFD, &numPacketsToSend, sizeof(int));                  // send the number of packets client is going to receive
    while(n==-1)
    	int n = write(clientFD, &numPacketsToSend, sizeof(int));
    char *msgToSend = (char*)malloc(numPacketsToSend*RESPONSE_BYTES);         // allocate buffer
    strcpy(msgToSend, str);                                                  
    int i;
    //send by breaking into packets
...
char* recieveMsgFromClient(int clientFD) {
    ...
    int n = read(clientFD, &numPacketsToReceive, sizeof(int));              // find how many packets will be received
    if(n <= 0) {                                                            // if no bytes to be received then close the connection
        shutdown(clientFD, SHUT_WR);
        return NULL;
    }
    char *str = (char*)malloc(numPacketsToReceive*REQUEST_BYTES);          //allocate memory to buffer
    memset(str, 0, numPacketsToReceive*REQUEST_BYTES);
    char *str_p = str;                                                      //str_p is 0 why return str_p not str
    int i;
	...
	while(flag)
	{
		...
		switch(choice)
		{
			case 1:
				...
				break;                                                                      //why not free(bal) like free(str) in case2
			case 2:
...
void updateTrans(char *username,int choice,double balance)
{
	...
	sprintf(line1,"%.*s %c %f\n",(int)strlen(asctime(localtime(&ltime)))-1,asctime(localtime(&ltime)),c,balance);  //wtf
...
void adminRequests(int client_fd)
{
	...
	while(1)
	{
		...
		else if(checkUser(buff))
		{
			...
			if(query(userreq,client_fd)==EXIT)                      //what abt other cases in query
				break;
		}
		else
